...
if __name__ == '__main__':
    ...
    file_handler = logging.handlers.RotatingFileHandler(filename='dummy-flask-app.log', mode='w', maxBytes=10000000, backupCount=0)
    ...
    logger = logging.basicConfig(level=logging.INFO,
                        format=f'[{app_name}] - [%(asctime)s] - %(levelname)-10s - %(message)s',
                        datefmt='%d-%b-%y %H:%M:%S',
                        handlers=[file_handler, stdout_handler])

    # Applying color to the output logs
    coloredlogs.install(fmt=f'[{app_name}] - [%(asctime)s] - %(levelname)-10s - %(message)s',
                        datefmt='%d-%b-%y %H:%M:%S',
                        logger=logger)


    logging.info('Starting flask server application ...  ')
